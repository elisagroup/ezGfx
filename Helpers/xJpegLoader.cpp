...
static int jpeg_read_bit() {   // read bit
	register int i;
...
static int jpeg_read_bits(int num) {   // read num bit
	register int i,j;
	for(i = 0, j = 0; i < num; i++) {
		j <<= 1;
		j |= jpeg_read_bit();
...
static int jpeg_bit2int(int bit,int i) {   // convert bit code to int
	if((i & (1 << (bit - 1))) > 0) return i;
	return -(i ^ ((1 << bit) - 1));
}

static int jpeg_huffmancode(jpeg_huffman_table_t *table) { // get Huffman code
...
static void jpeg_idct(float *data) {   // aa&n algorithm inverse DCT
...
static int jpeg_readmarkers() {    // read jpeg markers
...
static void jpeg_decompress() {    // decompress jpeg file (Baseline algorithm)
...
static void jpeg_ycbcr2rgb() { // convert YCbCr image to RGB
	int i,Y,Cb,Cr,R,G,B;
...
static void jpeg_gray2rgb() {  // grayscale image to RGB
	int i,j;
	for(i = 0; i < jpeg_file_s.width * jpeg_file_s.height << 2; i += 4) {
...
bool xJpegLoader::canLoad(const char* extension, FILE* file) const
{
...
unsigned char* xJpegLoader::loadImage(FILE* file, bool keepAlpha)
{
	memset(&jpeg_file_s,0,sizeof(jpeg_file_s));
	jpeg_file_s.file = file;
...
int xJpegLoader::getWidth() const
{
	return jpeg_file_s.width;
}

int xJpegLoader::getHeight() const
{

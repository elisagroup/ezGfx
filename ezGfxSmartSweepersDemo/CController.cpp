	...
	if (m_iTicks++ < CParams::iNumTicks)
	{
		for (int i=0; i<m_NumSweepers; ++i)
		{
			...
			if (!m_vecSweepers[i].Update(m_vecMines))
			{
				...
				cerr << "Error: Wrong amount of NN inputs!" << endl;
...
void CController::Render()
{
	...
	ezGfxDrawText(5, 0, s.c_str());

	//do not render if running at accelerated speed
	if (!m_bFastRender)
	{
		ezGfxSetStrokeParameters(1, 0, 150, 0);
		...
		for (int i=0; i<m_NumMines; ++i)
		{
			...
			//draw the mines
			ezGfxMoveTo((int)mineVB[0].x, (int)mineVB[0].y);

			for (int vert=1; vert<mineVB.size(); ++vert)
			{
				ezGfxLineTo((int)mineVB[vert].x, (int)mineVB[vert].y);
			}

			ezGfxLineTo((int)mineVB[0].x, (int)mineVB[0].y);
			
		}
       		
		//we want the fittest displayed in red
		ezGfxSetStrokeParameters(1, 255, 0, 0);
		...
		for (int i=0; i<m_NumSweepers; i++)
		{
			...
			ezGfxMoveTo((int)sweeperVB[0].x, (int)sweeperVB[0].y);

			for (int vert=1; vert<4; ++vert)
			{
				ezGfxLineTo((int)sweeperVB[vert].x, (int)sweeperVB[vert].y);
			}

			ezGfxLineTo((int)sweeperVB[0].x, (int)sweeperVB[0].y);

			//draw the sweeper right track
			ezGfxMoveTo((int)sweeperVB[4].x, (int)sweeperVB[4].y);
			...
			ezGfxLineTo((int)sweeperVB[4].x, (int)sweeperVB[4].y);

			ezGfxMoveTo((int)sweeperVB[8].x, (int)sweeperVB[8].y);
			ezGfxLineTo((int)sweeperVB[9].x, (int)sweeperVB[9].y);

			ezGfxMoveTo((int)sweeperVB[10].x, (int)sweeperVB[10].y);
...
void CController::PlotStats()
{
	...
	ezGfxDrawText(5, 20, s.c_str());

	s = "Average Fitness: " + ftos(m_pGA->AverageFitness());
	ezGfxDrawText(5, 40, s.c_str());
    ...
    ezGfxMoveTo(0, cyClient);
    
    for (int i=0; i<m_vecBestFitness.size(); ++i)
    {
       ezGfxLineTo(x, cyClient - VSlice*m_vecBestFitness[i]);
	...
	ezGfxSetStrokeParameters(1, 0, 0, 255);

	ezGfxMoveTo(0, cyClient);
    
    for (int i=0; i<m_vecAvFitness.size(); ++i)
    {
       ezGfxLineTo((int)x, (int)(cyClient - VSlice*m_vecAvFitness[i]));
